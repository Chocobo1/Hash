...
namespace RIPEMD_256_Hash
{
	class RIPEMD_256
	{
		...
		public:
			...
			inline explicit RIPEMD_256();

			inline void reset();
			inline RIPEMD_256& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			inline std::string toString() const;
			inline std::vector<RIPEMD_256::Byte> toVector() const;

			inline RIPEMD_256& addData(const Span<const Byte> inData);
			inline RIPEMD_256& addData(const void *ptr, const long int length);
	...
	RIPEMD_256& RIPEMD_256::finalize()
	{
	...
	RIPEMD_256& RIPEMD_256::addData(const Span<const Byte> inData)
	{
	...
	RIPEMD_256& RIPEMD_256::addData(const void *ptr, const long int length)
	{
		// gsl::span::index_type = long int
		return addData({reinterpret_cast<const Byte*>(ptr), length});
	}

	void RIPEMD_256::addDataImpl(const Span<const Byte> data)
	{
	...
	using RIPEMD_256 = RIPEMD_256_Hash::RIPEMD_256;
}
