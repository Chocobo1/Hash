...
namespace RIPEMD_256_Hash
{
	class RIPEMD_256
	{
		...
		public:
			...
			void reset();
			RIPEMD_256& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			std::string toString() const;
			std::vector<RIPEMD_256::Byte> toVector() const;

			RIPEMD_256& addData(const Span<const Byte> inData);
			RIPEMD_256& addData(const void *ptr, const long int length);
