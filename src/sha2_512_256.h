...
namespace SHA2_512_256_NS
{
	class SHA2_512_256
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_512_256& finalize();  // after this, only `operator T()`, `reset()`, `toArray()`, `toString()`, `toVector()` are available
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_512_256& addData(Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_512_256& addData(const void *ptr, std::size_t length);
			template <std::size_t N>
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_512_256& addData(const Byte (&array)[N]);
			template <typename T, std::size_t N>
			SHA2_512_256& addData(const T (&array)[N]);
			template <typename T>
			SHA2_512_256& addData(Span<T> inSpan);

			friend constexpr bool operator==(const SHA2_512_256 &left, const SHA2_512_256 &right)
			{
				for (int i = 0; i < 8; ++i)
				{
		...
		private:
			...
			INLINE_CLASS_VARIABLE_CHOCOBO1_HASH uint64_t kTable[80] =
			{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_512_256& SHA2_512_256::finalize()
	{
	...
	inline std::string SHA2_512_256::toString() const
	{
	...
	inline std::vector<SHA2_512_256::Byte> SHA2_512_256::toVector() const
	{
		const auto digest = toArray();
		return {digest.begin(), digest.end()};
	}

	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_512_256::ResultArrayType SHA2_512_256::toArray() const
	{
	...
	template <typename T>
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_512_256::operator T() const noexcept
	{
		static_assert(std::is_unsigned<T>::value, "");
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_512_256& SHA2_512_256::addData(const Span<const Byte> inData)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_512_256& SHA2_512_256::addData(const void *ptr, const std::size_t length)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline void SHA2_512_256::addDataImpl(const Span<const Byte> data)
	{
...
#else
			const auto round = [this, &wTable](uint64_t &a, uint64_t &b, uint64_t &c, uint64_t &d, uint64_t &e, uint64_t &f, uint64_t &g, uint64_t &h, const unsigned int t) -> void
#endif
			{
...
{
	template <>
	struct hash<Chocobo1::SHA2_512_256>
	{
		CONSTEXPR_CPP17_CHOCOBO1_HASH size_t operator()(const Chocobo1::SHA2_512_256 &hash) const noexcept
		{
			return hash;
		}
	};
