...
namespace RIPEMD_160_NS
{
	class RIPEMD_160
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH RIPEMD_160& finalize();  // after this, only `operator T()`, `reset()`, `toArray()`, `toString()`, `toVector()` are available
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH RIPEMD_160& addData(Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH RIPEMD_160& addData(const void *ptr, std::size_t length);
			template <std::size_t N>
			CONSTEXPR_CPP17_CHOCOBO1_HASH RIPEMD_160& addData(const Byte (&array)[N]);
			template <typename T, std::size_t N>
			RIPEMD_160& addData(const T (&array)[N]);
			template <typename T>
			RIPEMD_160& addData(Span<T> inSpan);

			friend constexpr bool operator==(const RIPEMD_160 &left, const RIPEMD_160 &right)
			{
				for (int i = 0; i < 5; ++i)
				{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline RIPEMD_160& RIPEMD_160::finalize()
	{
	...
	inline std::string RIPEMD_160::toString() const
	{
	...
	inline std::vector<RIPEMD_160::Byte> RIPEMD_160::toVector() const
	{
		const auto digest = toArray();
		return {digest.begin(), digest.end()};
	}

	CONSTEXPR_CPP17_CHOCOBO1_HASH inline RIPEMD_160::ResultArrayType RIPEMD_160::toArray() const
	{
	...
	template <typename T>
	CONSTEXPR_CPP17_CHOCOBO1_HASH RIPEMD_160::operator T() const noexcept
	{
		static_assert(std::is_unsigned<T>::value, "");
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline RIPEMD_160& RIPEMD_160::addData(const Span<const Byte> inData)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline RIPEMD_160& RIPEMD_160::addData(const void *ptr, const std::size_t length)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline void RIPEMD_160::addDataImpl(const Span<const Byte> data)
	{
...
{
	template <>
	struct hash<Chocobo1::RIPEMD_160>
	{
		CONSTEXPR_CPP17_CHOCOBO1_HASH size_t operator()(const Chocobo1::RIPEMD_160 &hash) const noexcept
		{
			return hash;
		}
	};
