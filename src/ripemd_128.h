...
namespace RIPEMD_128_Hash
{
	class RIPEMD_128
	{
		...
		public:
			...
			inline explicit RIPEMD_128();

			inline void reset();
			inline RIPEMD_128& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			inline std::string toString() const;
			inline std::vector<RIPEMD_128::Byte> toVector() const;

			inline RIPEMD_128& addData(const Span<const Byte> inData);
			inline RIPEMD_128& addData(const void *ptr, const long int length);
	...
	RIPEMD_128& RIPEMD_128::finalize()
	{
	...
	RIPEMD_128& RIPEMD_128::addData(const Span<const Byte> inData)
	{
	...
	RIPEMD_128& RIPEMD_128::addData(const void *ptr, const long int length)
	{
		// gsl::span::index_type = long int
		return addData({reinterpret_cast<const Byte*>(ptr), length});
	}

	void RIPEMD_128::addDataImpl(const Span<const Byte> data)
	{
	...
	using RIPEMD_128 = RIPEMD_128_Hash::RIPEMD_128;
}
