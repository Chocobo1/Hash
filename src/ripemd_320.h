...
namespace RIPEMD_320_Hash
{
	class RIPEMD_320
	{
		...
		public:
			...
			inline explicit RIPEMD_320();

			inline void reset();
			inline RIPEMD_320& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			inline std::string toString() const;
			inline std::vector<RIPEMD_320::Byte> toVector() const;

			inline RIPEMD_320& addData(const Span<const Byte> inData);
			inline RIPEMD_320& addData(const void *ptr, const long int length);
	...
	using RIPEMD_320 = RIPEMD_320_Hash::RIPEMD_320;
}
