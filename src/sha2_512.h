...
namespace SHA2_512_Hash
{
	class SHA2_512
	{
		...
		public:
			...
			inline explicit SHA2_512();

			inline void reset();
			inline SHA2_512& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			inline std::string toString() const;
			inline std::vector<SHA2_512::Byte> toVector() const;

			inline SHA2_512& addData(const Span<const Byte> inData);
			inline SHA2_512& addData(const void *ptr, const long int length);
	...
	SHA2_512& SHA2_512::addData(const Span<const Byte> inData)
	{
	...
	SHA2_512& SHA2_512::addData(const void *ptr, const long int length)
	{
		// gsl::span::index_type = long int
		return addData({reinterpret_cast<const Byte*>(ptr), length});
	}

	void SHA2_512::addDataImpl(const Span<const Byte> data)
	{
	...
	using SHA2_512 = SHA2_512_Hash::SHA2_512;
}
