...
namespace SHA2_224_NS
{
	class SHA2_224
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& finalize();  // after this, only `operator T()`, `reset()`, `toArray()`, `toString()`, `toVector()` are available
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& addData(Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& addData(const void *ptr, std::size_t length);
			template <std::size_t N>
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& addData(const Byte (&array)[N]);
			template <typename T, std::size_t N>
			SHA2_224& addData(const T (&array)[N]);
			template <typename T>
			SHA2_224& addData(Span<T> inSpan);
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_224& SHA2_224::finalize()
	{
	...
	inline std::string SHA2_224::toString() const
	{
	...
	inline std::vector<SHA2_224::Byte> SHA2_224::toVector() const
	{
		const auto digest = toArray();
		return {digest.begin(), digest.end()};
	}

	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_224::ResultArrayType SHA2_224::toArray() const
	{
	...
	template <typename T>
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224::operator T() const noexcept
	{
		static_assert(std::is_unsigned<T>::value, "");
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_224& SHA2_224::addData(const Span<const Byte> inData)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_224& SHA2_224::addData(const void *ptr, const std::size_t length)
	{
	...
	template <std::size_t N>
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_224& SHA2_224::addData(const Byte (&array)[N])
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline void SHA2_224::addDataImpl(const Span<const Byte> data)
	{
...
{
	template <>
	struct hash<Chocobo1::SHA2_224>
	{
		CONSTEXPR_CPP17_CHOCOBO1_HASH size_t operator()(const Chocobo1::SHA2_224 &hash) const noexcept
		{
			return hash;
		}
	};
