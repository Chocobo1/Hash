...
namespace SM3_NS
{
	class SM3
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SM3& addData(Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH SM3& addData(const void *ptr, std::size_t length);
			template <std::size_t N>
			CONSTEXPR_CPP17_CHOCOBO1_HASH SM3& addData(const Byte (&array)[N]);
			template <typename T, std::size_t N>
			SM3& addData(const T (&array)[N]);
			template <typename T>
			SM3& addData(Span<T> inSpan);
