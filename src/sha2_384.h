...
namespace SHA2_384_Hash
{
	class SHA2_384
	{
		...
		public:
			...
			void reset();
			SHA2_384& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			std::string toString() const;
			std::vector<SHA2_384::Byte> toVector() const;

			SHA2_384& addData(const Span<const Byte> inData);
			SHA2_384& addData(const void *ptr, const long int length);
