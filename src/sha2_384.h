...
namespace SHA2_384_NS
{
	class SHA2_384
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_384& finalize();  // after this, only `operator T()`, `reset()`, `toArray()`, `toString()`, `toVector()` are available
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_384& addData(Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_384& addData(const void *ptr, std::size_t length);
			template <std::size_t N>
			CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_384& addData(const Byte (&array)[N]);
			template <typename T, std::size_t N>
			SHA2_384& addData(const T (&array)[N]);
			template <typename T>
			SHA2_384& addData(Span<T> inSpan);

			friend constexpr bool operator==(const SHA2_384 &left, const SHA2_384 &right)
			{
				for (int i = 0; i < 8; ++i)
				{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_384& SHA2_384::finalize()
	{
	...
	inline std::string SHA2_384::toString() const
	{
	...
	inline std::vector<SHA2_384::Byte> SHA2_384::toVector() const
	{
		const auto digest = toArray();
		return {digest.begin(), digest.end()};
	}

	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_384::ResultArrayType SHA2_384::toArray() const
	{
	...
	template <typename T>
	CONSTEXPR_CPP17_CHOCOBO1_HASH SHA2_384::operator T() const noexcept
	{
		static_assert(std::is_unsigned<T>::value, "");
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_384& SHA2_384::addData(const Span<const Byte> inData)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline SHA2_384& SHA2_384::addData(const void *ptr, const std::size_t length)
	{
	...
	CONSTEXPR_CPP17_CHOCOBO1_HASH inline void SHA2_384::addDataImpl(const Span<const Byte> data)
	{
...
{
	template <>
	struct hash<Chocobo1::SHA2_384>
	{
		CONSTEXPR_CPP17_CHOCOBO1_HASH size_t operator()(const Chocobo1::SHA2_384 &hash) const noexcept
		{
			return hash;
		}
	};
