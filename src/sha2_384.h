...
namespace SHA2_384_Hash
{
	class SHA2_384
	{
		...
		public:
			...
			inline explicit SHA2_384();

			inline void reset();
			inline SHA2_384& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			inline std::string toString() const;
			inline std::vector<SHA2_384::Byte> toVector() const;

			inline SHA2_384& addData(const Span<const Byte> inData);
			inline SHA2_384& addData(const void *ptr, const long int length);
	...
	SHA2_384& SHA2_384::addData(const Span<const Byte> inData)
	{
	...
	SHA2_384& SHA2_384::addData(const void *ptr, const long int length)
	{
		// gsl::span::index_type = long int
		return addData({reinterpret_cast<const Byte*>(ptr), length});
	}

	void SHA2_384::addDataImpl(const Span<const Byte> data)
	{
	...
	using SHA2_384 = SHA2_384_Hash::SHA2_384;
}
