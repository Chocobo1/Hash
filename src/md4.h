...
namespace MD4_Hash
{
	class MD4
	{
		...
		public:
			...
			inline void reset();
			inline MD4& finalize();  // after this, only `toString()`, `toVector()`, `reset()` are available

			inline std::string toString() const;
			inline std::vector<MD4::Byte> toVector() const;

			inline MD4& addData(const Span<const Byte> inData);
			inline MD4& addData(const void *ptr, const long int length);
	...
	MD4& MD4::addData(const Span<const Byte> inData)
	{
	...
	MD4& MD4::addData(const void *ptr, const long int length)
	{
		// gsl::span::index_type = long int
		return addData({reinterpret_cast<const Byte*>(ptr), length});
	}

	void MD4::addDataImpl(const Span<const Byte> data)
	{
