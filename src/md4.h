...
namespace MD4_NS
{
	class MD4
	{
		...
		public:
			...
			CONSTEXPR_CPP17_CHOCOBO1_HASH MD4& addData(Span<const Byte> inData);
			CONSTEXPR_CPP17_CHOCOBO1_HASH MD4& addData(const void *ptr, std::size_t length);
			template <std::size_t N>
			CONSTEXPR_CPP17_CHOCOBO1_HASH MD4& addData(const Byte (&array)[N]);
			template <typename T, std::size_t N>
			MD4& addData(const T (&array)[N]);
			template <typename T>
			MD4& addData(Span<T> inSpan);

			friend constexpr bool operator==(const MD4 &left, const MD4 &right)
			{
				for (int i = 0; i < 4; ++i)
				{
